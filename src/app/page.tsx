...
export default function Home() {
  ...
  async function checkSession() {
    ...
    if (data.user) {
      setIsLoggedIn(true);
      await Promise.all([loadPhrases("candidate"), loadMessageDrafts("draft")]);
    } else {
      setIsLoggedIn(false);
    }
  ...
  async function updateMessageDraftStatus(
    id: number,
    newStatus: "approved" | "rejected"
  ) {
    setMessage("");
      ...
      <main className="min-h-screen bg-slate-50 flex items-center justify-center p-6">
        <div className="w-full max-w-md rounded-2xl bg-white p-8 shadow-sm border border-slate-200">
          ...
          <p className="mt-2 text-sm text-slate-600">
            Log in to review phrase suggestions and message drafts.
          </p>
                    ...
                    <div className="flex gap-2">
                      {draftFilter === "draft" && (
                        <>
                          ...
                          <button
                            onClick={() =>
                              updateMessageDraftStatus(draft.id, "approved")
                            }
                            className="rounded-lg bg-green-700 px-4 py-2 text-sm font-medium text-white hover:bg-green-800"
                          >
                            Approve
                          ...
                          <button
                            onClick={() =>
                              updateMessageDraftStatus(draft.id, "rejected")
                            }
                            className="rounded-lg bg-slate-200 px-4 py-2 text-sm font-medium text-slate-800 hover:bg-slate-300"
                          >
                            Reject
